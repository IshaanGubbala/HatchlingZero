...
from reference.hz0h_bdh_symmetric_backward_torch import (
    bdh_symmetric_backward_attention,
    bdh_symmetric_backward_forward,
)
...
def test_symmetric_attention_matches_output_and_input_gradients():
    torch.manual_seed(5)
    ...
    candidate = bdh_symmetric_backward_attention(q_b, v_b, attention.freqs)
...
def test_full_model_matches_all_parameter_gradients():
    ...
    candidate_logits, candidate_loss = bdh_symmetric_backward_forward(
...
def test_one_adamw_update_matches_reference():
    ...
    _, candidate_loss = bdh_symmetric_backward_forward(
        candidate_model, idx, candidate_model.config.n_layer, targets
    )
    reference_loss.backward()
    candidate_loss.backward()
