...
- **Latest, 2026-08-25 — a compound architecture (frozen-identity state compression + SVD-warmstarted low-rank decoder) now beats exact BDH on quality, training speed, training memory, AND inference speed simultaneously** — real, cross-seed-checked, GPU-verified numbers in [Latest: the efficiency-architecture sweep and the compound win](#latest-the-efficiency-architecture-sweep-and-the-compound-win) below. This is the first result in the project that clears every one of those axes at once rather than trading one for another. **Real, important correction, 2026-08-26: that quality table was measured at a fast 5M-token toy scale.** Rerun at this project's own established real budget (25M tokens): the win survives (compound still beats baseline) but the margin shrinks to under 10% of what the 5M-token number implied — see the same section below for the real numbers and the honest single-seed caveat.
...
- **2026-08-26 — a sharp, user-supplied calculation caught a real benchmark bug, and the fix uncovered a genuine, large, zero-architecture-change production-serving win.** Naive batching (constructing one physically-batched tensor per decode step) was collapsing aggregate decode throughput from 156 tok/s at batch=1 down to 75 tok/s at batch=2, and OOMing by batch=8 on a 24GB card. A user-independently-computed analytic state size flagged the reported 23.27GB peak at batch=4 as ~15.7× larger than the real persistent state should be — investigated rather than accepted, and traced to a real bug: the benchmark never called `torch.cuda.reset_peak_memory_stats()`, so its "peak" reading was a stale running-max contaminated by earlier batch sizes, not that batch size's true cost (real corrected peak: 13.78GB, not 23.27GB). Testing the user's own proposed fix directly — **virtual batching**: N independent, single-request states round-robined through individual decode steps, no physically-batched tensor ever built (the standard LLM continuous-batching pattern) — aggregate throughput stays **flat at the batch=1 ceiling (~155-159 tok/s) all the way out to 32 concurrent resident requests**, using only 15.6GB, no collapse, no OOM. Turns "max ~4 concurrent requests" into "32+ concurrent requests at effectively full single-request throughput," with zero architecture change. See `results/local/hz0h_vb_subspace_decoder_virtual_batch_benchmark.json`.
...
**A second, bigger honest downgrade, 2026-08-26 — the table above is real, but it's a
5M-token toy scale.** Directly asked afterward: does this hold at a real training
budget? This project's own established "real" scale (Phase F, below) is 25M tokens —
5x the table above. Real runs at that budget, same seed/shape/methodology:

| | 5M tokens (table above) | 25M tokens (real budget) |
| --- | ---: | ---: |
| exact BDH baseline | 1.8585 | **1.4383** |
| compound | 1.7907 | **1.4326** |
| margin | −0.068 (−3.65%) | **−0.0057 (−0.40%)** |

Both numbers drop enormously at 25M tokens — confirmation that 5M tokens was a real,
severely undertrained regime for a ~300M-param model. **The compound win survives — it
is not a 5M-token artifact — but the margin shrinks to under 10% of its 5M-token size.**
Single seed at 25M tokens; this session's own cross-seed checks at 5M-token scale found
seed-to-seed noise of 0.017-0.020, larger than this entire 25M-token margin — so this
result reads as "a real, small, positive effect" rather than a statistically confirmed
one at the same confidence as the 5M-token, 2-seed numbers above. A second seed at 25M
tokens is the obvious next step, not yet run given the real time cost (~80 minutes per
arm). See `plans/HatchlingZero_BDH_Efficiency_Architecture_Plan_2026-08-24.md` §20.3.
