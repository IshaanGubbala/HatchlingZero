...
unsafe fn write_out(state: &MemoryState, out: &CMemoryStateOut) {
    ...
    std::slice::from_raw_parts_mut(out.confidence, state.confidence.len())
        .copy_from_slice(&state.confidence);
    std::slice::from_raw_parts_mut(out.age, state.age.len()).copy_from_slice(&state.age);
    std::slice::from_raw_parts_mut(out.protection, state.protection.len())
        .copy_from_slice(&state.protection);
    std::slice::from_raw_parts_mut(out.write_count, state.write_count.len())
        .copy_from_slice(&state.write_count);
    std::slice::from_raw_parts_mut(out.last_write_step, state.last_write_step.len())
        .copy_from_slice(&state.last_write_step);
    std::slice::from_raw_parts_mut(out.write_source, state.write_source.len())
        .copy_from_slice(&state.write_source);
}

#[no_mangle]
pub unsafe extern "C" fn hz0b_reset(
    batch: usize,
    num_slots: usize,
    key_dim: usize,
    value_dim: usize,
    out: *mut CMemoryStateOut,
) {
...
pub unsafe extern "C" fn hz0b_read(
    ...
    let idx_opt = if slot_idx.is_null() {
        None
    } else {
        Some(std::slice::from_raw_parts(slot_idx, state.batch))
    };
...
pub unsafe extern "C" fn hz0b_write(
    ...
    let idx_opt = if slot_idx.is_null() {
        ...
        Some(std::slice::from_raw_parts(slot_idx, batch))
    };
    let (new_state, slot, rejected) =
        mem::write(&state, key, value, strength, step, source, idx_opt);
...
#[no_mangle]
pub unsafe extern "C" fn hz0b_reinforce(
    state_in: *const CMemoryStateIn,
    slot_idx: *const i32,
    out_state: *mut CMemoryStateOut,
) {
...
#[no_mangle]
pub unsafe extern "C" fn hz0b_update(
    state_in: *const CMemoryStateIn,
    slot_idx: *const i32,
    new_value: *const f32,
    out_state: *mut CMemoryStateOut,
...
#[no_mangle]
pub unsafe extern "C" fn hz0b_protect(
    state_in: *const CMemoryStateIn,
    slot_idx: *const i32,
    strength: *const f32,
    out_state: *mut CMemoryStateOut,
...
#[no_mangle]
pub unsafe extern "C" fn hz0b_forget_or_decay(
    state_in: *const CMemoryStateIn,
    decay_rate: f32,
    out_state: *mut CMemoryStateOut,
) {
...
#[no_mangle]
pub unsafe extern "C" fn hz0b_delete(
    state_in: *const CMemoryStateIn,
    slot_idx: *const i32,
    out_state: *mut CMemoryStateOut,
) {
