...
impl RmsNorm {
    pub fn new(name: &str, dim: usize) -> Self {
        Self {
            weight: Parameter::filled(&format!("{name}.weight"), vec![dim], 1.0),
            dim,
            eps: 1e-6,
            cache_x: Vec::new(),
    ...
    pub fn backward(&mut self, grad_output: &[f32]) -> Vec<f32> {
        ...
        for row in 0..rows {
            ...
            for col in 0..dim {
                grad_input[row * dim + col] = self.weight.data[col] * inv_rms * g[col]
                    - self.weight.data[col] * x[col] * inv_rms.powi(3) * dot / dim as f32;
            }
        }
        grad_input
    }
...
impl SwiGlu {
    ...
    pub fn backward(&mut self, grad_output: &[f32]) -> Vec<f32> {
        ...
        let activated = silu_forward(&self.cache_gate);
        let grad_gate_act: Vec<f32> = grad_product
            .iter()
            .zip(self.cache_up.iter())
            .map(|(g, u)| g * u)
            .collect();
        let grad_gate = silu_backward(&self.cache_gate, &grad_gate_act);
        let grad_up: Vec<f32> = grad_product
            .iter()
            .zip(activated.iter())
            .map(|(g, a)| g * a)
            .collect();
...
impl Embedding {
    pub fn new(name: &str, vocab_size: usize, dim: usize, seed: u64) -> Self {
        Self {
            weight: Parameter::random(&format!("{name}.weight"), vec![vocab_size, dim], 0.02, seed),
            dim,
            cache_ids: Vec::new(),
        }
    }
    ...
    pub fn forward(&mut self, ids: &[usize]) -> Vec<f32> {
        ...
        for (row, &id) in ids.iter().enumerate() {
            out[row * self.dim..(row + 1) * self.dim]
                .copy_from_slice(&self.weight.data[id * self.dim..(id + 1) * self.dim]);
        }
        out
    }
        ...
        for head in 0..self.heads {
            for t in 0..steps {
                ...
                for s in 0..=t {
                    ...
                    for c in 0..self.head_dim {
                        acc +=
                            grad_mixed[self.idx(t, head, c)] * self.cache_v[self.idx(s, head, c)];
                    }
                    grad_weights_row[s] = acc;
                }
                let weighted_dot: f32 = (0..=t)
                    .map(|s| {
                        grad_weights_row[s] * self.cache_weights[(head * steps + t) * steps + s]
                    })
                    .sum();
                ...
                for s in 0..=t {
                    for c in 0..self.head_dim {
                        grad_k[self.idx(s, head, c)] +=
                            grad_scores_row[s] * self.cache_q[self.idx(t, head, c)] * scale;
                        grad_v[self.idx(s, head, c)] += self.cache_weights
                            [(head * steps + t) * steps + s]
                            * grad_mixed[self.idx(t, head, c)];
                    }
                }
            }
        }
        ...
        for step in 0..steps {
            for head in 0..self.heads {
                ...
                for c in 0..self.head_dim {
                    ...
                    grad_packed[head_base + 2 * self.head_dim + c] =
                        grad_v[self.idx(step, head, c)];
                }
            }
        }
...
impl Gdn2Block {
    pub fn new(name: &str, dim: usize, heads: usize, d_k: usize, d_v: usize, seed: u64) -> Self {
        ...
        Self {
            ...
            d_k,
            d_v,
            cache_projected: Vec::new(),
            cache_shape: None,
            cache_forward: None,
            cache_initial_state: Vec::new(),
        }
    }
    ...
    pub fn final_state(&self) -> Option<&[f32]> {
        self.cache_forward
            .as_ref()
            .map(|result| result.final_state.as_slice())
    }
    ...
    pub fn forward(&mut self, x: &[f32], steps: usize, initial_state: &[f32]) -> Vec<f32> {
        ...
        for step in 0..steps {
            for head in 0..self.heads {
                ...
                k[qk_base..qk_base + self.d_k]
                    .copy_from_slice(&projected[base + self.d_k..base + 2 * self.d_k]);
                v[v_base..v_base + self.d_v].copy_from_slice(
                    &projected[base + 2 * self.d_k..base + 2 * self.d_k + self.d_v],
                );
                let offset = 2 * self.d_k + self.d_v;
                decay[qk_base..qk_base + self.d_k]
                    .copy_from_slice(&projected[base + offset..base + offset + self.d_k]);
                erase[qk_base..qk_base + self.d_k].copy_from_slice(
                    &projected[base + offset + self.d_k..base + offset + 2 * self.d_k],
                );
                write[v_base..v_base + self.d_v]
                    .copy_from_slice(&projected[base + offset + 2 * self.d_k..base + per_head]);
            }
        }
        ...
        let result = gdn2_forward_f32(&shape, &q, &k, &v, &decay, &erase, &write, initial_state)
            .expect("gdn2 forward shape mismatch");
    ...
    pub fn backward(
        ...
        for step in 0..steps {
            for head in 0..self.heads {
                ...
                let qk_base = (step * self.heads + head) * self.d_k;
                let v_base = (step * self.heads + head) * self.d_v;
                q[qk_base..qk_base + self.d_k]
                    .copy_from_slice(&self.cache_projected[base..base + self.d_k]);
                k[qk_base..qk_base + self.d_k]
                    .copy_from_slice(&self.cache_projected[base + self.d_k..base + 2 * self.d_k]);
                v[v_base..v_base + self.d_v].copy_from_slice(
                    &self.cache_projected[base + 2 * self.d_k..base + 2 * self.d_k + self.d_v],
                );
                let offset = 2 * self.d_k + self.d_v;
                decay[qk_base..qk_base + self.d_k].copy_from_slice(
                    &self.cache_projected[base + offset..base + offset + self.d_k],
                );
                erase[qk_base..qk_base + self.d_k].copy_from_slice(
                    &self.cache_projected[base + offset + self.d_k..base + offset + 2 * self.d_k],
                );
                write[v_base..v_base + self.d_v].copy_from_slice(
                    &self.cache_projected[base + offset + 2 * self.d_k..base + per_head],
                );
            }
        }
        let grads = gdn2_backward_f32(
            &shape,
            &q,
            &k,
            &v,
        ...
        for step in 0..steps {
            for head in 0..self.heads {
                ...
                let qk_base = (step * self.heads + head) * self.d_k;
                let v_base = (step * self.heads + head) * self.d_v;
                grad_projected[base..base + self.d_k]
                    .copy_from_slice(&grads.grad_q[qk_base..qk_base + self.d_k]);
                grad_projected[base + self.d_k..base + 2 * self.d_k]
                    .copy_from_slice(&grads.grad_k[qk_base..qk_base + self.d_k]);
                grad_projected[base + 2 * self.d_k..base + 2 * self.d_k + self.d_v]
                    .copy_from_slice(&grads.grad_v[v_base..v_base + self.d_v]);
                let offset = 2 * self.d_k + self.d_v;
                grad_projected[base + offset..base + offset + self.d_k]
                    .copy_from_slice(&grads.grad_decay_logits[qk_base..qk_base + self.d_k]);
                grad_projected[base + offset + self.d_k..base + offset + 2 * self.d_k]
                    .copy_from_slice(&grads.grad_erase_logits[qk_base..qk_base + self.d_k]);
                grad_projected[base + offset + 2 * self.d_k..base + per_head]
                    .copy_from_slice(&grads.grad_write_logits[v_base..v_base + self.d_v]);
            }
        }
...
impl Block {
    pub fn new_gdn2(
        ...
        Self {
            ...
            mixer: Mixer::Gdn2(Gdn2Block::new(
                &format!("{name}.gdn2"),
                dim,
                heads,
                d_k,
    ...
    pub fn new_attention(name: &str, dim: usize, heads: usize, d_ff: usize, seed: u64) -> Self {
        Self {
            norm1: RmsNorm::new(&format!("{name}.norm1"), dim),
            mixer: Mixer::Attention(CausalAttention::new(
    ...
    pub fn forward(
        ...
        let mlp_out = self
            .mlp
            .forward(&self.norm2.forward(&residual, steps), steps);
        let _ = dim;
    ...
    pub fn backward(
        ...
        let grad_input: Vec<f32> = grad_residual
            .iter()
            .zip(grad_norm1.iter())
            .map(|(a, b)| a + b)
            .collect();
...
impl TinyModel {
    pub fn new(
        ...
        for index in 0..layers {
            ...
            if attention_indices.contains(&index) {
                ...
                blocks.push(Block::new_gdn2(
                    &name, dim, heads, d_k, d_v, d_ff, block_seed,
                ));
            }
        }
    ...
    pub fn loss_and_backward(&mut self, token_ids: &[usize], targets: &[usize]) -> f32 {
        ...
        let grad_hidden_from_head = self
            .embedding
            .lm_head_backward(&grad_logits, &hidden, steps);
    ...
    pub fn flat_gradients(&mut self) -> Vec<f64> {
        self.parameters_mut()
            .iter()
            .flat_map(|p| p.grad.iter().map(|&g| g as f64))
            .collect()
    }
}
    ...
    pub fn update(&mut self, parameters: &mut [&mut Parameter]) {
        ...
        for parameter in parameters.iter_mut() {
            ...
            for i in 0..parameter.data.len() {
                ...
                parameter.data[i] -= self.lr
                    * (m_hat / (v_hat.sqrt() + self.eps) + self.weight_decay * parameter.data[i]);
            }
        }
    }
}
...
mod tests {
    ...
    fn finite_difference_check<F: Fn(&mut TinyModel) -> f32>(
        model: &mut TinyModel,
        loss_fn: F,
        tolerance: f32,
    ) {
    ...
    fn bf16_precision_model_trains_and_stays_finite() {
        ...
        for _ in 0..8 {
            ...
            assert!(
                loss.is_finite(),
                "loss went non-finite at BF16 weight precision"
            );
            for parameter in model.parameters_mut() {
                assert!(
                    parameter.grad.iter().all(|g| g.is_finite()),
                    "{}: non-finite gradient at BF16 precision",
                    parameter.name
                );
            }
            optimizer.update(&mut model.parameters_mut());
            for parameter in model.parameters_mut() {
                assert!(
                    parameter.data.iter().all(|v| v.is_finite()),
                    "{}: non-finite parameter after AdamW at BF16 precision",
                    parameter.name
                );
            }
            last_loss = loss;
