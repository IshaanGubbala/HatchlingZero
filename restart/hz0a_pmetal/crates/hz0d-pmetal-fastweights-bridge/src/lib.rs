...
unsafe fn from_c(s: &CFastWeightStateIn) -> FastWeightState {
    FastWeightState {
        ...
        dim: s.dim,
        rank: s.rank,
        a_fast: std::slice::from_raw_parts(s.a_fast, s.sessions * s.num_layers * s.dim * s.rank).to_vec(),
        b_fast: std::slice::from_raw_parts(s.b_fast, s.sessions * s.num_layers * s.rank * s.dim).to_vec(),
...
unsafe fn write_out(state: &FastWeightState, out: &CFastWeightStateOut) {
    ...
    std::slice::from_raw_parts_mut(out.update_count, state.update_count.len()).copy_from_slice(&state.update_count);
}

#[no_mangle]
pub unsafe extern "C" fn hz0d_reset(sessions: usize, num_layers: usize, dim: usize, rank: usize, a_fast_init: *const f32, out: *mut CFastWeightStateOut) {
...
#[no_mangle]
pub unsafe extern "C" fn hz0d_apply(state_in: *const CFastWeightStateIn, x: *const f32, base_weight: *const f32, base_bias: *const f32, layer: usize, out_y: *mut f32) {
...
pub unsafe extern "C" fn hz0d_update(
    state_in: *const CFastWeightStateIn, session: usize, layer: usize,
    grad_a: *const f32, grad_b: *const f32, lr: f32, max_delta_norm: f32,
...
#[no_mangle]
pub unsafe extern "C" fn hz0d_decay(state_in: *const CFastWeightStateIn, decay_rate: f32, out: *mut CFastWeightStateOut) {
    let state = from_c(&*state_in);
...
#[no_mangle]
pub unsafe extern "C" fn hz0d_effective_delta(state_in: *const CFastWeightStateIn, session: usize, layer: usize, out_delta: *mut f32) {
    let state = from_c(&*state_in);
