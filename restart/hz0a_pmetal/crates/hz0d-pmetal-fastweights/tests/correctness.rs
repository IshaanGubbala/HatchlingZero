...
fn reset_gives_exactly_zero_realized_delta_asymmetric_init() {
    ...
    assert!(
        state.b_fast.iter().all(|&v| v == 0.0),
        "b_fast must be exactly zero at reset"
    );
    let delta = effective_delta(&state, 0, 0);
    assert!(
        delta.iter().all(|&v| v == 0.0),
        "A @ 0 must be exactly zero regardless of A"
    );
}
...
fn update_strictly_reduces_a_toy_loss() {
    ...
    for (i, (&before, &g)) in state.b_fast.iter().zip(grad_b.iter()).enumerate() {
        ...
        assert!(
            (updated.b_fast[i] - expected).abs() < 1e-4,
            "b_fast[{i}] should move by exactly -lr*grad before clipping dominates"
        );
    }
...
fn clip_bounds_realized_delta_norm_regardless_of_gradient_magnitude() {
    ...
    assert!(
        norm <= 1.0 + 1e-3,
        "realized delta norm {norm} exceeds max_delta_norm=1.0"
    );
}
...
fn snapshot_and_rollback_restore_state_exactly() {
    ...
    assert_ne!(
        mutated.b_fast, start.b_fast,
        "sanity: the update must have actually changed state"
    );
...
fn batched_update_touches_only_the_named_session() {
    let sessions = 3;
    let a_init: Vec<f32> = (0..sessions * LAYERS * DIM * RANK)
        .map(|i| 0.01 * (i as f32))
        .collect();
    ...
    assert_eq!(
        &updated.a_fast[0..a_len],
        &state.a_fast[0..a_len],
        "session 0 must be untouched"
    );
    assert_eq!(
        &updated.a_fast[2 * a_len..3 * a_len],
        &state.a_fast[2 * a_len..3 * a_len],
        "session 2 must be untouched"
    );
    assert_eq!(
        &updated.b_fast[0..b_len],
        &state.b_fast[0..b_len],
        "session 0's b_fast must be untouched"
    );
    assert_eq!(
        &updated.b_fast[2 * b_len..3 * b_len],
        &state.b_fast[2 * b_len..3 * b_len],
        "session 2's b_fast must be untouched"
    );
    // session 1 must have actually changed
    assert_ne!(
        &updated.b_fast[b_len..2 * b_len],
        &state.b_fast[b_len..2 * b_len],
        "session 1 must have changed"
    );
    assert_eq!(
        updated.update_count,
        vec![0, 1, 0],
        "only session 1's update_count should increment"
    );
}
...
fn apply_is_deterministic_and_independent_per_session_in_one_batched_call() {
    ...
    assert_ne!(
        &y1[0..DIM],
        &y1[DIM..2 * DIM],
        "different sessions with different deltas must produce different outputs"
    );
}
