...
kernel void hz0e_moe_swiglu(
    ...
    for (uint j = 0; j < dff; ++j) {
        ...
        float up = fallback ? fallback_biases[dim + j] : expert_biases[expert * (2 * expert_d_ff + dim) + expert_d_ff + j];
        for (uint i = 0; i < dim; ++i) {
            if (fallback) {
                gate += fallback_weights[j * dim + i] * input[input_base + i];
                up += fallback_weights[dim * dim + j * dim + i] * input[input_base + i];
            } else {
        ...
        if (fallback) value += fallback_weights[2 * dim * dim + out * dim + j] * hidden;
