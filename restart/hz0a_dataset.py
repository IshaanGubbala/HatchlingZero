...
class StreamingResumablePackedDataset:
    """Offset-indexed JSONL iterator for packed corpora too large to load eagerly."""

    def __init__(self, packed_jsonl_path: str | Path, shuffle_seed: int = 0, cursor: DatasetCursor | None = None):
        self.path = Path(packed_jsonl_path)
        self.offsets = []
        ...
        if not self.offsets:
            raise ValueError("packed JSONL dataset must contain at least one sequence")
        if cursor is None:
            order = list(range(len(self.offsets)))
            random.Random(shuffle_seed).shuffle(order)
            cursor = DatasetCursor(order=order)
        if sorted(cursor.order) != list(range(len(self.offsets))):
            raise ValueError("cursor order must be a permutation of the dataset")
        self.cursor = cursor
    ...
    def next_batch(self, batch_size: int) -> np.ndarray:
        ...
        with self.path.open("rb") as reader:
            ...
            for index in indices:
                ...
                sequences.append(json.loads(reader.readline()))
    ...
    @classmethod
    def from_snapshot(cls, packed_jsonl_path: str | Path, snapshot: dict[str, object]) -> "StreamingResumablePackedDataset":
        return cls(packed_jsonl_path, cursor=DatasetCursor(**snapshot))
