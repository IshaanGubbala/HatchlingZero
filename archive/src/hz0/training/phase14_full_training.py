...
class TrainingHarness:
    ...
    def step(self, grads_list) -> None:
        """Average accumulated gradients, clip, and apply optimizer update."""
        if not grads_list:
            return
        ...
        try:
            for epoch in range(num_epochs):
                ...
                for step_idx, (tokens, targets) in enumerate(train_batches):
                    ...
                    tokens_seen += int(tokens.size)
                    microbatch_count += 1
                    microbatches_in_window += 1

                    # Accumulate until we have a full grad_accum buffer,
                    # then average, clip, and step. This always fires
                    # regardless of max_steps — max_steps only controls
                    # the stopping condition below.
                    if len(grads_buffer) >= self.grad_accum:
                        self.step(grads_buffer)
                        grads_buffer = []
                        optimizer_step += 1

                        if (
                            optimizer_step > 0
                            and optimizer_step % self.checkpoint_every == 0
                        ):
                            avg_train_loss = epoch_loss / max(1, microbatches_in_window)
                            val_loss = self._evaluate(val_batches[:10])

                            elapsed = time.time() - start_time
                            throughput = tokens_seen / max(elapsed, 1e-8)

                            # Capture peak Metal memory. mx.metal.get_peak_memory()
                            # returns bytes since last reset; reset before each
                            # window so the metric reflects this window only.
                            peak_mem_bytes = 0
                            try:
                                import mlx.metal as _metal  # type: ignore
                                peak_mem_bytes = int(_metal.get_peak_memory())
                                _metal.reset_peak_memory()
                            except Exception:
                                peak_mem_bytes = 0
                            ...
                            is_full = (
                                ...
                                and (optimizer_step % self.save_optimizer_every == 0)
                            )
                            save_mlx_checkpoint(
                                self.output_dir,
                                optimizer_step,
                            ...
                            print(
                                f"Epoch {epoch + 1} "
                                f"opt_step {optimizer_step:4d} "
                                f"micro {microbatch_count:5d} "
                                f"tok {tokens_seen:>9,}: "
                    ...
                    # Stop only AFTER completing a full grad_accum cycle.
                    if max_steps is not None and optimizer_step >= max_steps:
                        print(
                            f"[max_steps={max_steps} reached] terminating "
                            f"at optimizer_step {optimizer_step} "
                            f"(microbatch {microbatch_count}, "
                    ...
                    # Epoch completed without break — flush any partial
                    # grad_accum buffer so the final microbatches still
                    # contribute an optimizer update.
                    if grads_buffer:
                        self.step(grads_buffer)
                        grads_buffer = []
                        optimizer_step += 1
                    # Continue to next epoch (skips the break below).
                    # Without this, the unconditional break after the
                    # else block would kill multi-epoch training.
                    continue

                # Only reached when inner for broke (max_steps reached)
                break

            completed_cleanly = True
            print(
                f"[done] {self.model_name}: "
                f"{optimizer_step} optimizer steps, "
                f"{microbatch_count} microbatches, "
                f"{tokens_seen:,} tokens, "
            ...
            self.metrics.append({"status_marker": status, "optimizer_step": optimizer_step})
            self._flush_metrics()
            try:
                save_mlx_checkpoint(
                    self.output_dir,
                    ...
                    self.optimizer,
                    cfg=self.cfg,
                    metrics={"final": True, "optimizer_step": optimizer_step, "status": status},
                    model_only=False,
                )
                ...
                print(
                    ...
                    f"final ckpt at optimizer_step {optimizer_step}",
                    flush=True,
                )
...
def parse_args(argv=None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Optional explicit seed for mx.random.seed. Critical for "
             "the backend A/B comparison (Use the same seed for MLX-ref "
             "and fused-Metal runs to keep initialization comparable).",
    )
...
def main(argv=None) -> int:
    ...
    # Smoke-test gate: refuse to launch a full run without an explicit
    # step count or smoke flag. --max-steps=200 caps us at 200 optimizer
    # steps; --smoke-test caps at 5. Either prevents accidentally
    # launching a multi-hour run with a broken backend.
    if not args.smoke_test and args.max_steps is None:
        print(
            "[refuse] full training run requires either --max-steps "
            "(recommended: 200 for any backend first run) or "
            "--smoke-test. Pass one of these to proceed.",
            flush=True,
        )
        return 10

    # Seed the underlying RNG if requested. Critical for the fused-vs-MLX
    # backend comparison so the two runs see identical initialization.
    if args.seed is not None:
        mx.random.seed(args.seed)
        print(f"[seed] mx.random.seed({args.seed}) applied", flush=True)

    # When USE_FUSED_METAL=1 is set, the fused-Metal forward path
    # is used during training. The new gdn2_fused_trainable wrapper
    # uses @mx.custom_function with a VJP that delegates gradients
    # to the proven MLX reference backward — so training works correctly.
    # Inference mode (INFERENCE_MODE=1) bypasses the VJP entirely.
    if os.environ.get("USE_FUSED_METAL", "0") == "1":
        print(
            "[fused-metal] USE_FUSED_METAL=1 detected; using fused "
            "Metal forward + MLX-reference backward VJP for training.",
            flush=True,
        )
        # Fused-forward side-check: validate the fused-Metal path
        # produces finite outputs even though we don't use it during
        # training. Catches fused-path NaN regressions that the
        # training-time MLX-ref path would otherwise mask.
        try:
            from hz0.metal_gdn2.kernels.gdn2_fused_metal import (
                gdn2_fused_forward,
            )
            os.environ["INFERENCE_MODE"] = "1"
            # Pin a fixed seed AND run 3 calls so transient Metal/flush
            # NaN issues surface. State_out is ALSO finiteness-checked:
            # its NaN would silently propagate into the next forward.
            B, T, H, Dk, Dv = 2, 32, 12, 64, 64
            mx.random.seed(0)
            q = mx.random.uniform(shape=(B, T, H, Dk)).astype(mx.float32)
            k = mx.random.uniform(shape=(B, T, H, Dk)).astype(mx.float32)
            v = mx.random.uniform(shape=(B, T, H, Dv)).astype(mx.float32)
            d = mx.sigmoid(
                mx.random.uniform(shape=(B, T, H, Dk))
            ).astype(mx.float32)
            e = mx.sigmoid(
                mx.random.uniform(shape=(B, T, H, Dk))
            ).astype(mx.float32)
            w = mx.sigmoid(
                mx.random.uniform(shape=(B, T, H, Dv))
            ).astype(mx.float32)
            state = mx.zeros((B, H, Dv, Dk), dtype=mx.float32)
            per_call_finite: List[bool] = []
            for _i in range(3):
                fused_out, fused_state = gdn2_fused_forward(
                    q, k, v, d, e, w, initial_state=state
                )
                mx.eval(fused_out, fused_state)
                out_finite = bool(mx.all(mx.isfinite(fused_out)))
                state_finite = bool(mx.all(mx.isfinite(fused_state)))
                per_call_finite.append(out_finite and state_finite)
                # Chain state across calls so recurrent NaN
                # propagation surfaces even if call 1's output is
                # technically finite — e.g. overflow that only
                # breaks at depth > 1.
                state = fused_state
                print(
                    f"[fused-side-check] call {_i + 1}/3 "
                    f"out_finite={out_finite} state_finite={state_finite}",
                    flush=True,
                )
            fused_finite = all(per_call_finite)
            print(
                f"[fused-side-check] 3 chained calls at shape "
                f"(B={B},T={T},H={H},Dk={Dk},Dv={Dv}) "
                f"all_calls_finite={fused_finite}",
                flush=True,
            )
            if not fused_finite:
                print(
                    "[fused-side-check] FAIL: at least one fused call "
                    "returned non-finite output or state; aborting.",
                    flush=True,
                )
                return 11
        finally:
            # RESTORE INFERENCE_MODE to its pre-side-check value.
            # The side-check set it to "1" to exercise the fused
            # inference forward; training must see it as "0" (or unset).
            os.environ["INFERENCE_MODE"] = "0"
            # RESTORE the user's chosen RNG seed for the actual
            # training run. Without this restore the side-check's
            # `mx.random.seed(0)` clobbers args.seed for all
            # subsequent MLX random ops, contaminating the run.
            if args.seed is not None:
                mx.random.seed(args.seed)
    else:
        print(
            "[training-path] MLX reference forward (USE_FUSED_METAL=0).",
            flush=True,
        )
