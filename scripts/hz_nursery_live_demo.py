...
Runs the 4 landed stages in curriculum order -- L0, then L1, then L2,
then L3 -- in one process, so a viewer watching from the start sees the
model progress through the Nursery the way the plan itself sequences it.
"""
...
def main() -> None:
    ...
    parser.add_argument("--loop", action="store_true", help="repeat the whole L0-L3 curriculum forever")
