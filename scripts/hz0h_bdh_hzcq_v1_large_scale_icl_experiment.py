...
def run_transformer(args, K: int):
...
def main() -> None:
    ...
    K = args.k_symbols
    print(f"=== {args.model}, {args.train_steps} steps, K={K}, N_DEMOS={args.n_demos} ===", flush=True)
    if args.model == "sh":
        eval_acc, history, n_params = run_sh(args, K)
        tag = "S+H"
    else:
        eval_acc, history, n_params = run_transformer(args, K)
        tag = "Transformer"
