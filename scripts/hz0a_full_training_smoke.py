...
def run(config_path: Path, steps: int, batch_size: int, sequence_length: int, seed: int, learning_rate: float) -> dict:
    ...
    for step in range(1, steps + 1):
        tokens = torch.randint(config.vocab_size, (batch_size, sequence_length + 1))
...
def main() -> None:
    ...
    report = run(args.config, args.steps, args.batch_size, args.sequence_length, args.seed, args.learning_rate)
    if args.output:
