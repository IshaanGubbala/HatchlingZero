"""HZ Phase 2R-E (`plans/HZ Phase 2R State Redesign Plan.md`): real
quality check for the COMBINED value-bottleneck + INT8-state reduction
-- both already independently validated at 0% measured degradation
(`docs/restart/hz0h_phase2r_value_bottleneck_results.md`,
`docs/restart/hz0h_phase3_state_quantization_results.md`), tested
together for the first time here, not assumed to compose for free.
"""
from __future__ import annotations

import sys
from pathlib import Path

sys.path.insert(0, str(Path(__file__).resolve().parents[1]))

import json

import numpy as np
import torch

from reference.hz0h_bdh_h5_memory_tasks import make_passkey_sequence
from reference.hz0h_bdh_train_torch import shifted_target_batch
from reference.hz0h_bdh_vb_torch import (
    BDHVB,
    BDHVBConfig,
    bdh_vb_stream_chunk,
    bdh_vb_stream_chunk_int8_state,
    init_bdh_vb_states,
    init_bdh_vb_states_int8,
)

VOCAB_SIZE = 32
PREFIX_LEN = 4
FILLER_LEN = 16
PASSKEY_RANGE = 8
N_LAYER = 2
N_EMBD = 32
N_HEAD = 4
MLP_MULT = 8
STEPS = 1200  # matched budget established in the 2R-B writeup (400 steps undertrains VB's extra P/O)
BATCH_SIZE = 16
SEED = 0


def train_bdh_vb(d_state: int) -> BDHVB:
    torch.manual_seed(SEED)
    config = BDHVBConfig(n_layer=N_LAYER, n_embd=N_EMBD, n_head=N_HEAD, mlp_internal_dim_multiplier=MLP_MULT, vocab_size=VOCAB_SIZE, dropout=0.0, d_state=d_state)
    model = BDHVB(config)
    optimizer = torch.optim.AdamW(model.parameters(), lr=2e-3, weight_decay=0.01)
    rng = np.random.default_rng(SEED)
    for _step in range(STEPS):
        seqs = []
        for _ in range(BATCH_SIZE):
            seq, answer = make_passkey_sequence(rng, vocab_size=VOCAB_SIZE, prefix_len=PREFIX_LEN, filler_len=FILLER_LEN, passkey_range=PASSKEY_RANGE)
            seqs.append(seq + [answer])
        batch = torch.tensor(seqs, dtype=torch.long)
        x, y = shifted_target_batch(batch)
        _logits, loss = model(x, targets=y)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
    model.eval()
    return model


@torch.no_grad()
def evaluate_fp32_state(model: BDHVB, num_examples: int = 200, seed: int = 7000) -> float:
    rng = np.random.default_rng(seed)
    correct = 0
    for _ in range(num_examples):
        seq, answer = make_passkey_sequence(rng, vocab_size=VOCAB_SIZE, prefix_len=PREFIX_LEN, filler_len=FILLER_LEN, passkey_range=PASSKEY_RANGE)
        idx = torch.tensor([seq], dtype=torch.long)
        states = init_bdh_vb_states(model, 1)
        for t in range(idx.shape[1] - 1):
            states, _ = bdh_vb_stream_chunk(model, states, idx[:, t:t + 1], start_position=t)
        _s, logits = bdh_vb_stream_chunk(model, states, idx[:, -1:], start_position=idx.shape[1] - 1)
        correct += int(int(logits[0, -1].argmax()) == answer)
    return correct / num_examples


@torch.no_grad()
def evaluate_int8_state(model: BDHVB, num_examples: int = 200, seed: int = 7000) -> float:
    rng = np.random.default_rng(seed)
    correct = 0
    for _ in range(num_examples):
        seq, answer = make_passkey_sequence(rng, vocab_size=VOCAB_SIZE, prefix_len=PREFIX_LEN, filler_len=FILLER_LEN, passkey_range=PASSKEY_RANGE)
        idx = torch.tensor([seq], dtype=torch.long)
        states = init_bdh_vb_states_int8(model, 1)
        for t in range(idx.shape[1] - 1):
            states, _ = bdh_vb_stream_chunk_int8_state(model, states, idx[:, t:t + 1], start_position=t)
        _s, logits = bdh_vb_stream_chunk_int8_state(model, states, idx[:, -1:], start_position=idx.shape[1] - 1)
        correct += int(int(logits[0, -1].argmax()) == answer)
    return correct / num_examples


def state_bytes(n_layer: int, n_head: int, n_embd: int, mlp_mult: int, value_width: int, dtype_bytes: int) -> int:
    N = n_embd * mlp_mult // n_head
    return n_layer * n_head * N * value_width * dtype_bytes


def main() -> None:
    exact_bdh_state_bytes = state_bytes(N_LAYER, N_HEAD, N_EMBD, MLP_MULT, N_EMBD, 4)  # exact BDH, fp32
    results = {}
    for d_state in (N_EMBD, N_EMBD // 4, N_EMBD // 8):
        label = f"d_state_{d_state}"
        print(f"Training HZ-BDH-VB, d_state={d_state}...")
        model = train_bdh_vb(d_state)
        fp32_accuracy = evaluate_fp32_state(model)
        int8_accuracy = evaluate_int8_state(model)
        vb_fp32_bytes = state_bytes(N_LAYER, N_HEAD, N_EMBD, MLP_MULT, d_state, 4)
        vb_int8_bytes = state_bytes(N_LAYER, N_HEAD, N_EMBD, MLP_MULT, d_state, 1)
        results[label] = {
            "d_state": d_state,
            "fp32_state_accuracy": fp32_accuracy,
            "int8_state_accuracy": int8_accuracy,
            "int8_degradation_vs_fp32": fp32_accuracy - int8_accuracy,
            "reduction_vs_exact_bdh_fp32": exact_bdh_state_bytes / vb_int8_bytes,
        }
        print(json.dumps(results, indent=2))

    print(json.dumps(results, indent=2))


if __name__ == "__main__":
    main()
