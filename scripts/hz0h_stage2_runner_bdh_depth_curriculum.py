"""HZ Phase 6 / Training B (`plans/HatchlingZero_Reality_Plan.md` section
9.2, "Recurrent-Depth Curriculum"): trains exact BDH with the number of
shared-weight layer-loop iterations increasing over training (e.g.
2 -> 4 -> 6 -> 8), instead of paying for full recurrent depth from
step 0 -- Training A (`scripts/hz0h_stage2_runner_bdh.py`, already run
twice at 25M/seed 7/8, `docs/restart/hz0h_core1_quality_25m_results.md`)
is the fixed-depth baseline this compares against.

Real motivation directly from this session's own prior finding
(`docs/restart/hz0h_phase5_variable_depth_results.md` Update 3): a
NAIVE i.i.d.-random depth-sampling schedule on the tiny synthetic
multi-hop task did NOT converge at all (0.08-0.29 accuracy, near
chance) even on the easiest case, while a narrow-to-wide CURRICULUM
schedule fixed it completely (0.97-1.00 accuracy at every depth). This
script applies the same curriculum SHAPE to real language-model
pretraining at the 25M-param HZ-Core-1 scale, to test Phase 6's own
real questions: does curriculum accelerate convergence, stabilize deep
iteration, or reduce training energy relative to fixed-depth full BPTT?

Same CLI/data-loading/checkpoint/report-JSON conventions as
`scripts/hz0h_stage2_runner_bdh.py` for direct comparability, with one
addition: `--curriculum-stages` (a comma-separated list of
`token_boundary:n_iterations` pairs, e.g.
`6250000:2,12500000:4,18750000:6,25000000:8`) replacing the fixed
`--n-layer`-driven forward pass with `bdh_variable_depth_forward` at
the depth for the current token count. `model.config.n_layer` is still
set (required for weight-shape-independent parts of BDH's construction)
but is NOT what controls the forward pass here -- `n_iterations` from
the curriculum schedule is, matching `bdh_variable_depth_forward`'s own
decoupling of loop bound from `config.n_layer`.
"""
...
def parse_curriculum(spec: str, target_tokens: int) -> list[tuple[int, int]]:
    """`"boundary:depth,boundary:depth,..."` -> sorted [(token_boundary,
    n_iterations), ...]. The LAST stage's boundary should equal
    target_tokens (not enforced, just a real footgun to watch for -- a
    boundary short of target_tokens silently keeps using the last
    stage's depth for the remainder, which is often fine/intentional
...
def main() -> None:
    ...
    parser.add_argument("--n-layer", type=int, default=6, help="used only for BDH's weight-shape construction; the forward pass's real iteration count is controlled by --curriculum-stages")
    ...
    parser.add_argument("--activation-policy", choices=("auto", "recompute", "store"), default="auto", help="BDH activation retention policy. auto uses shared-round recomputation on CUDA and stored activations elsewhere.")
    parser.add_argument("--checkpoint-segment-size", type=int, default=1, help="Shared BDH rounds per recomputed segment. 1 minimizes peak memory.")
    parser.add_argument("--activation-checkpointing", action="store_true", help="Deprecated compatibility alias for --activation-policy recompute.")
    ...
    if args.activation_checkpointing:
        args.activation_policy = "recompute"
    ...
    memory_log = args.run_dir / "bdh_depth_curriculum_stage2_memory.jsonl"
    checkpoint = args.run_dir / "bdh_depth_curriculum_stage2_checkpoint"
    ...
    with args.data.open() as train, args.validation_data.open() as validation:
        ...
        while tokens_seen < args.target_tokens:
            ...
            item = {"step": step, "tokens_seen": tokens_seen, "n_iterations": n_iterations, "loss": float(loss.detach()), "gradient_norm": grad_norm, "lr": last_lr, "wall_time": time.perf_counter() - started, "epoch_or_data_pass": epoch_counter[0], "peak_memory_bytes": peak_memory_bytes(device)}

            is_new_best = False
            if step % args.validation_interval == 0 or tokens_seen >= args.target_tokens:
                # Real evaluation depth: whatever depth training is CURRENTLY
                # at, not always the final depth -- this tracks convergence
                # AT each curriculum stage honestly, rather than always
                # measuring against a depth the model hasn't reached yet.
    ...
    # Final validation at the FULL final curriculum depth, for direct
    # comparability against Training A's fixed-depth best_validation_loss
    # -- the in-loop validation above tracks CURRENT training depth
    # honestly, but the headline number needs to be measured the same way
    # Training A's is (full depth, real inference-time setting).
    final_depth = curriculum_stages[-1][1]
    final_full_depth_validation_loss = evaluate_fixed_validation(final_depth)

    report = {
        "backend": "torch", "device": str(device), "architecture": "bdh_depth_curriculum", "dtype": args.dtype,
    ...
    (args.run_dir / "bdh_depth_curriculum_stage2.json").write_text(json.dumps(report, indent=2), encoding="utf-8")
