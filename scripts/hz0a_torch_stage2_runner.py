...
def main() -> None:
    ...
    parser.add_argument("--mixer", choices=("gdn2", "gdn3", "gdn2_fix"), default="gdn2", help="Recurrent mixer for hybrid-architecture non-attention layers. 'gdn2' is HZ-0A's current locked-spec mixer. 'gdn3' is the candidate delta-rule mixer from docs/restart/hz0a_gdn3_candidate_design.md (reference/hz0a_gdn3_candidate_mixer_torch.py) -- real, positive evidence at small scale (a genuine associative-recall-with-overwrite win, tied generic-text perplexity) per that doc's own verdict, but still single-seed/small-scale, not yet validated as a wholesale replacement for the frozen HZ-0A Stage 2 spec. Parameter-count-matched with 'gdn2' by design. 'gdn2_fix' is the corrected true Gated DeltaNet-2 from plans/GDN-2_Fix.md (reference/hz0a_gdn2_fix_torch.py) -- independent vector erase/write gates and genuine key-conditioned delta correction, distinct from (and a fix for) the original 'gdn2' math; that doc reports full holdout validation loss beating a matched transformer (3.208 vs 4.378). Only affects attention_layer_indices-excluded layers; attention layers are unchanged either way. Only applies to --architecture hybrid.")
    ...
    parser.add_argument("--compile-mode", choices=("default", "reduce-overhead", "max-autotune"), default="default", help="torch.compile mode when --compile-step is enabled; fair comparisons must use the same mode in every arm.")
    ...
    if args.architecture == "transformer":
        ...
        if args.mixer in ("gdn3", "gdn2_fix") and (args.chunked_scan or args.fla_recurrence):
            raise ValueError(f"--chunked-scan and --fla-recurrence are original-GDN-2-specific fast paths (they don't match {args.mixer}'s math); not applicable with --mixer {args.mixer}")
    ...
    if args.compile_step:
        if args.architecture == "hybrid" and args.mixer == "gdn2" and args.chunked_scan:
            ...
            elif args.mixer == "gdn2_fix":
                # Same whole-chunk-loop compile pattern as gdn2/gdn3 above: one
                # compiled artifact shared (class attribute) across every layer
                # instance, verified 0.0-diff-modulo-float-reassociation against
                # the eager gdn2_fix_scan path before this wiring was added
                # (see reference/hz0a_gdn2_fix_torch.py's _gdn2_fix_sequential).
                from reference.hz0a_gdn2_fix_torch import GDN2FixMixer
                GDN2FixMixer._seq_fn = staticmethod(torch.compile(GDN2FixMixer._seq_fn))
            ...
            train_step_fn = torch.compile(model, mode=args.compile_mode)
