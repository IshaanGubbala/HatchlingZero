#!/usr/bin/env python3
"""Isolated compiled CUDA comparison of raw and RoPE-hoisted exact BDH."""
...
def _run_arm(args: argparse.Namespace) -> dict:
    ...
    def selected(model_arg, token_ids, target_ids):
        if args.arm == "raw":
            return model_arg(token_ids, target_ids)
        return bdh_hoisted_rope_forward(model_arg, token_ids, target_ids)

    forward = torch.compile(selected, fullgraph=True)
    ...
    def step() -> torch.Tensor:
        ...
        _logits, loss = forward(model, idx, targets)
        loss.backward()
        optimizer.step()
        return loss.detach()
    ...
    return {
        ...
        "last_loss": losses[-1],
        "finite": bool(torch.isfinite(torch.tensor(losses)).all())
...
def _args() -> argparse.Namespace:
    ...
    parser.add_argument("--arm", choices=("raw", "hoisted"), default="raw")
...
def main() -> None:
    ...
    report = {
        ...
        "steps": args.steps,
        "arms": arms,
        "hoisted_over_raw_throughput_ratio": (
            arms["hoisted"]["tokens_per_second"]
            / arms["raw"]["tokens_per_second"]
        ),
        "hoisted_over_raw_peak_memory_ratio": (
            arms["hoisted"]["peak_memory_bytes"]
            / arms["raw"]["peak_memory_bytes"]
        ),
    }
