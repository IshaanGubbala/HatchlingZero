...
from reference.hz0h_bdh_block_gated_torch import (BDHBlockGated, BDHBlockGatedConfig,
    ...
    bdh_block_gated_annealed_direct_split_v_compact_gate_forward,
...
def main():
    ...
    p.add_argument('--gate-layout',choices=('legacy','compact'),default='legacy',help='compact preserves the head-shared gate algebra while avoiding head expansion; direct_split_v only')
    ...
    if a.gate_layout=='compact' and a.value_path!='direct_split_v':raise ValueError('compact gate layout requires direct_split_v')
    ...
    gated_forward = bdh_block_gated_annealed_direct_split_v_chunk_gla_forward if a.attention_kernel == 'chunk_gla' else bdh_block_gated_annealed_direct_split_v_compact_gate_forward if a.gate_layout == 'compact' else bdh_block_gated_annealed_direct_split_v_forward if a.value_path == 'direct_split_v' else bdh_block_gated_annealed_forward
    ...
    report={'backend':'torch','device':str(dev),'hardware_id':hardware,'effective_batch_tokens':a.batch_size*a.sequence_length,'compile_step':False,'compile_mode':None,'fused_optimizer':a.fused_optimizer,'architecture':'block_gated_bdh_direct_split_v_chunk_gla_derivative' if a.attention_kernel == 'chunk_gla' else 'block_gated_bdh_direct_split_v_derivative' if a.value_path == 'direct_split_v' else 'block_gated_bdh_derivative','exact_bdh':False,'claim_eligible':False,'value_path':a.value_path,'attention_kernel':a.attention_kernel,'gate_layout':a.gate_layout,'dtype':a.dtype,'parameter_count':sum(p.numel() for p in model.parameters()),'block_size':a.block_size,'curriculum_stages':stages,'steps':step,'tokens_seen':tokens,'target_tokens':a.target_tokens,'budget_complete':tokens>=a.target_tokens,'best_validation_loss':best,'metrics':metrics,'checkpoint':str(checkpoint),'training_seconds':time.perf_counter()-started,'tokens_per_second':tokens/max(time.perf_counter()-started,1e-9),'peak_memory_bytes':peak(dev),'initialization_seed':a.seed,'final_parameter_sha256':fingerprint(model)}
