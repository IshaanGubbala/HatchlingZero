...
follow-up to the Physics coreference ablation). Same program shape as
CS ("x is {a}", "y is {b}"), but the question asks for ONE bound VALUE
directly ("what is x") instead of composing both via addition -- a
control cell to confirm single-fact value retrieval from a 2-fact
program isn't itself a bottleneck, isolating retrieval from
composition before testing retrieval of an ENTITY REFERENCE instead
(`hz_school0_entity_select_train.py`). Reuses `cs_program_forward`
completely unchanged -- it is already generic (statement list +
question -> arithmetic_head classification), only the generator and
target differ from the sum task.
"""
...
from hatchling_world.language.tokenizer import NurseryTokenizer, NUMBERS
from hatchling_world.school.generator import generate_value_retrieval_episode
...
def main() -> None:
    ...
    model = HZLanguageModel(vocab_size=tok.vocab_size, d_model=args.d_model, memory_slots=args.memory_slots,
                             ...
                             n_arith_labels=len(NUMBERS))
    print(f"[value-retrieval] vocab_size={tok.vocab_size} n_params={sum(p.numel() for p in model.parameters())}", flush=True)
    ...
    for step in range(args.steps):
        ...
        if (step + 1) % args.eval_every == 0:
            ...
            print(f"[value-retrieval] step={step+1}/{args.steps} train_acc={sum(recent)/len(recent):.3f} "
