"""HZ Next-Phase Plan Phase B2: real training runner for the selective
synaptic-state-write-gate VB arm (`reference/hz0h_bdh_vb_selective_torch.py`),
using the now-locked recurrent-depth curriculum. Same CLI/data-loading/
checkpoint/report-JSON conventions as
`scripts/hz0h_stage2_runner_bdh_vb_depth_curriculum.py` (plain VB, no
gate), for direct comparison at the same D/4 width Phase B locked
...
def main() -> None:
    ...
    parser.add_argument("--n-layer", type=int, default=6, help="used only for weight-shape construction; the forward pass's real iteration count is controlled by --curriculum-stages")
    ...
    parser.add_argument("--d-state-divisor", type=int, default=4, help="VB d_state = n_embd // this. Default 4 matches Phase B's locked choice.")
    ...
    memory_log = args.run_dir / "bdh_vb_selective_depth_curriculum_stage2_memory.jsonl"
    checkpoint = args.run_dir / "bdh_vb_selective_depth_curriculum_stage2_checkpoint"
    ...
    bdh_vb_selective_config = BDHVBSelectiveConfig(
    ...
    model = BDHVBSelective(bdh_vb_selective_config).to(device=device, dtype=torch_dtype)
    ...
    (args.run_dir / "bdh_vb_selective_depth_curriculum_stage2.json").write_text(json.dumps(report, indent=2), encoding="utf-8")
