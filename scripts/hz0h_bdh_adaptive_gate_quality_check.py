...
def gate_stats(model, args, device):
    """Real gate-value distribution on a held-out batch, not just the
    controller's own bias term -- the whole point of making it adaptive
    is that it should vary by input; a flat distribution near g_init
    would mean it learned nothing state-dependent."""
    from reference.hz0h_bdh_adaptive_gate_torch import _adaptive_g, _address
    import torch.nn.functional as F
    with args.validation_data.open() as handle, torch.no_grad(), autocast_context(args, device):
        data = read_batch(handle, args.batch_size, args.sequence_length, device, [0])
...
def main() -> None:
    ...
    print(f"[adaptive_gate] validation_loss={val_loss} params={params/1e6:.2f}M elapsed={elapsed:.0f}s "
          f"n_refresh={args.n_refresh}/{args.n_layer} gate_stats={gstats}", flush=True)
    ...
    args.out.write_text(json.dumps({
        ...
        "g_init": args.g_init, "gate_stats": gstats,
