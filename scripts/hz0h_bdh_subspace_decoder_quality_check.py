#!/usr/bin/env python3
"""Real training-quality check for Tier 4 item 23 (Subspace BDH,
factored decoder), same methodology as every quality-check run tonight
(seed=7, hz0h_bytes_25m data, 5M tokens, batch=8/seq=256, adamw,
bfloat16, gradient checkpointing, same depth curriculum) -- only the
model differs, so val_loss is directly comparable to exact BDH (1.8585)
and every other variant tested tonight without re-running those arms.
"""
    ...
    with args.data.open() as handle:
        for step in range(steps):
            ...
            if args.log_every and (step + 1) % args.log_every == 0:
                ...
                print(f"[train_subspace_decoder] step {step+1}/{steps} depth={depth} loss={float(loss):.4f} "
                      f"{rate:.0f} tok/s eta={eta:.0f}s", flush=True)
    synchronize(device)
    elapsed = time.perf_counter() - started
    print(f"[train_subspace_decoder] DONE {tokens} tokens in {elapsed:.0f}s final_loss={float(loss):.4f}", flush=True)
    model.eval()
...
def main() -> None:
    ...
    print(f"[subspace_decoder] validation_loss={val_loss} params={params/1e6:.2f}M", flush=True)

    report = {
        "config": {k: (str(v) if isinstance(v, Path) else v) for k, v in vars(args).items()},
        "subspace_rank": config.subspace_rank,
        "results": {"subspace_decoder": {"validation_loss": val_loss, "parameter_count": params, "training_seconds": elapsed}},
    }
