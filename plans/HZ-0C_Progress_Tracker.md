...
Updated: August 4, 2026 (C0-C4 complete; C6 hybrid promoted AND HZ-0B memory wired through the trigger graph; C8 Metal backward dispatch, Python-reference parity, AND a real model-level FFI integration for BOTH CPU and GPU kernels all done -- a real redundant-work bug fixed in the GPU shader along the way, and the honest benchmark that found it also precisely characterized the REAL remaining bottleneck (768x cross-thread redundancy from one-thread-per-output-element dispatch) rather than leaving "optimize later" vague; C7 tried a new feature family and hypothesis space, honestly confirmed a real plateau; C9's end-to-end quality/cost/latency/adversarial-failure report is complete. Only C8's now-concretely-specified grouped/cache-optimized dispatch and C7's plateau remain as named open production items.)
...
| C8 | PMetal implementation | **Forward + backward Metal dispatch, Python-reference parity, AND a real model-level FFI integration all verified; only performance optimization (grouped/cache-optimized dispatch) remains open** | `restart/hz0a_pmetal/crates/hz0a-pmetal-kernel/src/lib.rs`, `restart/hz0a_pmetal/crates/hz0a-pmetal-gpu/src/lib.rs`, `restart/hz0a_pmetal/crates/hz0a-pmetal-bridge/src/lib.rs`, `reference/hz0c_pmetal_bridge.py`, `docs/restart/hz0c_c8_pmetal_attention_results.md`, `docs/restart/hz0c_c8_pmetal_backward_and_parity_results.md`, `docs/restart/hz0c_c8_model_level_integration_results.md`, `scripts/hz0c_c8_ffi_latency_benchmark.py`, `tests/reference/test_hz0c_pmetal_bridge.py`. Native Metal backward dispatch (`MetalConditionalAnchorAttentionBackward`, single-threaded, correctness-first) exists alongside forward, both GPU-vs-CPU parity verified including a real two-batch shared-weight case. A cross-language fixture check against the actual Python `masked_anchor_attention` reference found and fixed a real bug: neither kernel multiplied the final output by the trigger value (a non-triggered position leaked `out_bias`) -- fixed in all 4 places (CPU/GPU x forward/backward). A second divergence for FRACTIONAL trigger values was found, characterized, and deliberately left unfixed with an explicit binary-trigger-only contract (Python's soft-trigger masking is a training-time-only property PMetal's inference kernel does not reproduce). **2026-08-04: model-level integration is now real for BOTH CPU and GPU kernels** -- `hz0a-pmetal-bridge` builds as a real `cdylib` exposing a flat-pointer C ABI (CPU) and a reusable opaque-handle API (GPU, `hz0c_metal_conditional_attention_create/_destroy/_forward`), `reference/hz0c_pmetal_bridge.py` calls both via `ctypes` (no PyO3/maturin), verified correct against the real Python reference (5 Python + 6 Rust tests, reusing the existing cross-language fixture). A real safety bug (undersized-buffer FFI read = undefined behavior) was found while writing the rejection test and fixed with Python-side shape validation before any pointer crosses the FFI boundary. A real redundant-work bug in the GPU forward SHADER was also found and fixed (query projection and per-source score were recomputed up to `2+head_dim` times each; now cached and computed once, same numerical result, all parity tests still pass). Honest performance result, measured not assumed: the CPU FFI kernel is **35-190x SLOWER** than MLX at dim=768; the GPU FFI kernel is WORSE STILL at higher trigger rates/sequence lengths (**13.8 SECONDS** at seq=128/100% trigger vs. CPU's 118ms) -- this precisely characterized the REAL dominant bottleneck: one Metal thread per OUTPUT ELEMENT means all 768 `D`-dimension threads sharing a token redundantly recompute the identical per-token score cache 768 times over. "Grouped/cache-optimized dispatch" now has a concrete, named fix (one threadgroup per token, shared-memory score cache across the `D` threads, the same pattern already proven for the GDN2 backward kernel) rather than being an open-ended label. |
...
- [x] Model-level integration (`docs/restart/hz0c_c8_model_level_integration_results.md`) -- real `ctypes`-based Python<->Rust FFI bridge for both CPU and GPU kernels, verified correct; honestly measured slower than MLX at production scale for both (CPU 35-190x, GPU up to ~50,000x at high trigger rate/seq length due to a newly-characterized 768x cross-thread redundancy). Grouped/cache-optimized dispatch (threadgroup-per-token shared-memory score cache) is the one remaining, now concretely specified, open C8 item.
