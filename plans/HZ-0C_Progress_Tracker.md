...
3. ~~Improve C9 trigger quality with a different causal objective or feature family.~~ Tried three independent ways, 2026-08-04, `docs/restart/hz0c_c9_attention_pattern_feature_results.md`: a new feature family (byte-identical result), a new hypothesis space/MLP (worse), and a new pairwise ranking objective (statistically tied). All three converge on the same ~0.51-0.52 recall ceiling -- read as a genuine information ceiling on the causal features available at inference time relative to the offline token-loss teacher, not an unfound fix. Real remaining candidate, not attempted: a genuinely different TEACHER signal (not token_loss_score).
...
| C7 | controller training | **Reference controller stable at a plateau confirmed real by THREE independent, convergent negative results -- feature family, hypothesis space, AND training objective all tried and rejected** | `scripts/hz0c_c7_rl_trigger_controller.py`, `scripts/hz0c_c7_multi_seed_report.py`, `docs/restart/hz0c_c7_rl_trigger_controller_results.md`, `docs/restart/hz0c_c9_attention_pattern_feature_results.md`. The bounded three-seed report remains **0.4800347 +/- 0.0256113** event recall and **0.4650347 +/- 0.0256113** reward at 15%. Longer RL, higher RL learning rate, and bounded causal-teacher blends did not improve it. Attempt 1 (feature family): `attention_pattern_features`, real content-aware realized-attention signal, received substantial real distillation weight (norm 0.71/2.13) but held-out quality was BYTE-IDENTICAL to baseline (0.5182/0.1068). Attempt 2 (hypothesis space): the same features through an MLP were WORSE (0.4531/0.0951). Attempt 3 (2026-08-04, objective): `fit_ranking_controller` -- a pairwise RankNet-style ranking loss instead of pointwise BCE, since every real consumer only uses score ORDER for top-k selection, never calibration; verified correct on a synthetic sanity check, then evaluated on the same held-out protocol -- best-tuned result **0.5208/0.1068**, statistically tied with BCE's 0.5182/0.1068, not a real improvement. **Three independent levers, one convergent ceiling**: this is now read as a genuine information ceiling (causal features cannot approach the offline token-loss teacher's 0.75-0.83 ceiling, which uses the real future token), not an unfound fix. |
...
- [ ] C7: the controller is bounded and nontrivial -- met; "matches the reference" in the sense of closing the gap to the offline teacher is plateaued, not improving further (tried a new feature family and hypothesis space, both confirmed the plateau rather than fixing it).
