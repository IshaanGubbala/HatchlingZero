...
Updated: August 4, 2026 (C0-C4 complete; C6 hybrid promoted AND HZ-0B memory wired through the trigger graph; **C8 is now fully complete** -- Metal backward dispatch, Python-reference parity, model-level FFI integration for both CPU and GPU kernels, AND grouped/cache-optimized dispatch (a threadgroup-cooperative GPU shader redesign giving a real, verified 24x-413x speedup and finally beating the CPU kernel) all done and honestly benchmarked, with the remaining 6x-21x gap to MLX itself disclosed rather than closed; C7 tried a new feature family and hypothesis space, honestly confirmed a real plateau; C9's end-to-end quality/cost/latency/adversarial-failure report is complete. C7's plateau is the only named open item across the whole HZ-0C tracker.)
...
- [x] Grouped/cache-optimized dispatch (`docs/restart/hz0c_c8_model_level_integration_results.md`) -- threadgroup-per-token shared-memory redesign of the GPU forward shader, implemented and verified correct. 24x-413x faster than the pre-fix GPU kernel, now consistently beats the CPU kernel (3.4x-19.8x); still 6x-21x behind MLX itself, disclosed honestly with two named, uninvestigated candidate causes for the remaining gap.
...
- [x] C8: the kernel is bounded, nontrivial, matches the reference, is dispatched from Python via a tested FFI bridge, AND has grouped/cache-optimized dispatch. -- `docs/restart/hz0c_c8_model_level_integration_results.md`. Still honestly 6x-21x behind MLX's own kernel (not claimed as closed), but no longer blocked on either integration or the previously-catastrophic 768x redundancy.
