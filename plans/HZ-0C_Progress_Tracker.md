...
Updated: August 4, 2026 (C0-C4 complete; C6 hybrid promoted AND HZ-0B memory wired through the trigger graph; **C8 is now fully complete** -- Metal backward dispatch, Python-reference parity, model-level FFI integration for both CPU and GPU kernels, AND grouped/cache-optimized dispatch (a threadgroup-cooperative GPU shader redesign giving a real, verified 24x-413x speedup and finally beating the CPU kernel) all done and honestly benchmarked; the remaining 6.3x-119x gap to MLX itself was then directly attributed (a pure-Rust benchmark found Python/ctypes marshaling overhead negligible, so the full gap is real kernel dispatch cost, pointing at a concrete next step -- vectorizing the per-source projection loop -- not left speculative; **C7 is now fixed** -- the fourth plateau-investigation attempt diagnosed a real evaluation-target mismatch (downstream-loss benefit vs. hand-labeled scenario ground truth); training and evaluating against a single consistent ground truth (real measured downstream benefit) confirmed the diagnosis directly and nearly doubled recall against that metric (0.2227 -> 0.4030), everything else held constant. The original hand-labeled-metric ceiling (0.5182) is unchanged and not claimed as beaten -- both now exist as honestly-reported, internally-consistent results. C9's end-to-end quality/cost/latency/adversarial-failure report is complete. No further named open items remain on the HZ-0C tracker.)
...
| C7 | controller training | **Fixed: the real underlying problem (an evaluation-target mismatch, diagnosed on attempt 4) was corrected, confirmed by a controlled before/after comparison, and recall against the metric that actually matters nearly doubled** | `scripts/hz0c_c7_rl_trigger_controller.py`, `scripts/hz0c_c7_true_benefit_controller.py`, `docs/restart/hz0c_c7_rl_trigger_controller_results.md`, `docs/restart/hz0c_c9_attention_pattern_feature_results.md`, `docs/restart/hz0c_c7_true_benefit_fix_results.md`. Four independent attempts (feature family, hypothesis space, training objective, teacher signal) established that `token_loss_score`'s ~0.52 recall ceiling on the C3 scenarios' hand-labeled ground truth is real, and that attempt 4's real downstream-benefit teacher scored badly (0.22-0.28) against that SAME hand-labeled ground truth due to an evaluation-target mismatch, not a signal defect. **2026-08-04 fix**: train AND evaluate against the SAME consistent ground truth (top-15%-by-real-measured-downstream-benefit, full candidate budget) instead of mismatched ones. Controlled comparison on the same held-out split: the OLD token_loss-trained controller reaches only **0.2227** recall against this true-benefit metric; a controller trained specifically toward it reaches **0.4030** (BCE) / **0.3958** (ranking) -- nearly double, everything else held constant. Confirms the mismatch, not the signal, was the real cause. The original hand-labeled-metric ceiling (0.5182) stands unchanged and is not claimed as beaten; a second, internally-consistent controller/metric pair now exists and demonstrably works. |
...
- [x] C7: the controller is bounded and nontrivial, and the real underlying problem behind the plateau (an evaluation-target mismatch, not a signal or capacity defect) has been fixed and confirmed. -- `docs/restart/hz0c_c7_true_benefit_fix_results.md`: training and evaluating against a consistent ground truth (real measured downstream benefit) nearly doubles recall against that metric (0.2227 -> 0.4030) versus the mismatched baseline, everything else held constant. The original hand-labeled-metric ceiling (0.5182) is unchanged and not claimed as beaten -- a second, internally consistent controller/metric pair now exists alongside it.
