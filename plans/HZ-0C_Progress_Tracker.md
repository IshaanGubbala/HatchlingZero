...
Updated: August 4, 2026 (C0-C4 complete; C6 hybrid promoted AND HZ-0B memory wired through the trigger graph; C8 Metal backward dispatch AND Python-reference parity done, real bug found and fixed along the way; C7 tried a new feature family and hypothesis space, honestly confirmed a real plateau; C9's end-to-end quality/cost/latency/adversarial-failure report is now complete, including an honest disclosed gap -- real latency savings are still gated on C8's model-level integration, the one production item now clearly remaining)
...
| C8 | PMetal implementation | **Forward + backward Metal dispatch AND Python-reference parity verified; grouped/cache-optimized dispatch and model-level integration open** | `restart/hz0a_pmetal/crates/hz0a-pmetal-kernel/src/lib.rs`, `restart/hz0a_pmetal/crates/hz0a-pmetal-gpu/src/lib.rs`, `docs/restart/hz0c_c8_pmetal_attention_results.md`, `docs/restart/hz0c_c8_pmetal_backward_and_parity_results.md`, `scripts/hz0c_generate_pmetal_conditional_attention_fixture.py`, `restart/hz0a_pmetal/crates/hz0a-pmetal-kernel/tests/parity_with_python_reference.rs`. Native Metal backward dispatch (`MetalConditionalAnchorAttentionBackward`, single-threaded, correctness-first) now exists alongside forward, both GPU-vs-CPU parity verified including a real two-batch shared-weight case. A NEW cross-language fixture check against the actual Python `masked_anchor_attention` reference (not just Rust-vs-Rust) found and fixed a real bug: neither the CPU nor GPU forward kernel multiplied the final output by the trigger value (a non-triggered position leaked `out_bias` through instead of producing exact zero) -- fixed in all 4 places (CPU/GPU x forward/backward), one existing test's hard-coded WRONG expectation (`0.25` instead of `0.0`) corrected with the history kept in its own comment. A second, real divergence for genuinely FRACTIONAL (non-binary) trigger values was found, characterized (~0.018 max diff), and deliberately left unfixed with an explicit binary-trigger-only contract documented on the kernel -- Python's soft-trigger additive masking is a training-time differentiability property PMetal's sparse inference kernel is not trying to reproduce, per this project's own Hard Constraint that real triggers are always discretized before deployment. |
...
- [x] PMetal/reference parity report (`docs/restart/hz0c_c8_pmetal_backward_and_parity_results.md`) -- CPU/GPU forward+backward now checked against the actual Python reference, not just Rust-vs-Rust; found and fixed a real bug. Grouped/cache-optimized dispatch and model-level integration remain separate open items.
...
- [ ] C7/C8: the trigger policy is bounded, nontrivial, and matches the reference. -- C8's KERNEL now provably matches the Python reference (`docs/restart/hz0c_c8_pmetal_backward_and_parity_results.md`); still open: C8 is not dispatched from the actual model forward pass (model-level integration), and C7's controller is plateaued, not improving further.
