...
| D3 | update-mechanism selection | **Complete, including a same-day fix** | `reference/hz0d_update_mechanisms.py`, `tests/reference/test_hz0d_update_mechanisms.py` (10 tests), `docs/restart/hz0d_d3_update_mechanism_results.md`. All 4 plan-named candidates implemented and compared on the identical D2 task with shared clipping. Original finding: delta prediction (closed-form least-squares + SVD truncation) won clean data (~4,000x faster, best quality) but collapsed under label noise (~135x worse than gradient descent, exact-interpolation overfitting). **Fixed same day**: ridge regularization on the normal equations (`(X^T X + ridge*I)^-1 X^T y`) closes the gap from ~135x-worse to statistically comparable (`0.9233` vs gradient descent's `0.8887` mean noisy held-out loss across 8 seeds) while keeping the full speed advantage -- a real, swept (8 ridge values), disclosed fix, not just a patched number. Hebbian's capacity limit (a_fast held fixed) is real, confirmed via a 12-config sweep, not fixable the same way. **Gradient descent remains the selected default** (continuity with D1/D2's already-validated mechanism), but ridge-regularized delta prediction is now a legitimate, fast, close alternative -- not a disqualified one -- named explicitly for future phases where latency matters more than the last few percent of quality. |
