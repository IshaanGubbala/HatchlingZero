...
| D3 | update-mechanism selection | **Complete, including two same-day fixes** | `reference/hz0d_update_mechanisms.py`, `tests/reference/test_hz0d_update_mechanisms.py` (11 tests), `docs/restart/hz0d_d3_update_mechanism_results.md`. All 4 plan-named candidates implemented and compared on the identical D2 task with shared clipping. Original finding: delta prediction (closed-form least-squares + SVD truncation) won clean data (~4,000x faster, best quality) but collapsed under label noise (~135x worse than gradient descent, exact-interpolation overfitting). **Fix 1**: ridge regularization on the normal equations closed the gap to statistically comparable, but only on one axis at a time (solve-dense-then-truncate has one regularization dial for a two-part problem). **Fix 2 (requested directly, "get ridge regularized to gradient accuracy")**: replaced solve-then-truncate with ridge-regularized ALTERNATING LEAST SQUARES (ALS) directly on the rank-2 factors -- verified via a noise-free synthetic sanity check first (recovers a known rank-2 matrix to <0.004 error), then swept ridge to find the point closest to gradient descent on BOTH clean AND noisy accuracy simultaneously: `ridge=0.27` lands within +2.8% (clean) and +2.7% (noisy) of gradient descent, measured on the same seeds, while staying ~480x faster (measured directly). Hebbian's capacity limit (a_fast held fixed) is real, confirmed via a 12-config sweep, not fixable the same way. **Gradient descent remains the selected default** (continuity with D1/D2's already-validated mechanism), with ALS delta prediction now a genuinely close, ~480x-faster alternative -- not just "not disqualified," within single-digit percent on both axes at once. |
