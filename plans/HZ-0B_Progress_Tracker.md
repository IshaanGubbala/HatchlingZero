...
| B5 | wait for a stable HZ-0A checkpoint | **Blocked, correctly** | Needs frozen architecture, reproducible tokenizer, stable PMetal implementation, verified gradients, reliable checkpoint loading, a trained checkpoint, and a known no-memory evaluation baseline. HZ-0A's Stage 2 (100M) is still running (transformer near-complete, hybrid in progress as of this update) -- not frozen yet. Per the plan's own text, "the isolated HZ-0B simulator can proceed in parallel," which is exactly what B0-B4 did. |
