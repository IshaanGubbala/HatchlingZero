...
- Overall phase: B0-B2 complete; B3+ not started. **The plan's own "first concrete milestone" is reached**: "a standalone, deterministic memory simulator with explicit read, write, update, protect, forget, delete, reset, and restore operations that passes synthetic tests for recall, overwrite, collision, and protection" -- `reference/hz0b_memory_simulator.py`, 20/20 tests passing, no LM attached.
