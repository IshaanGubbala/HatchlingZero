...
## 19.1 Real end-to-end check, 2026-08-29 -- compile-training's speedup did NOT replicate, gate replication inconclusive

Dispatched the canonical baseline (compound + `--gated-residual --gated-residual-single-stream --compile-training`, seed=13, 25M tokens) proposed as the next step: establish the new canonical config and answer the gate-replication and real-throughput questions in one run.

**`--compile-training` real result: no net win end-to-end.** seed=13+compile: 3766.2s wall-clock. seed=7 without compile (the original single-stream ablation, also RTX 5090): 3775.9s. A ~0.3% difference -- a wash, nowhere near the ~2x measured in isolated profiling (section 8.1). Real, likely explanation: the profiler measured a FIXED depth in a tight forward+backward loop, no recompilation. Real training uses the depth curriculum (stages e.g. 2->4->6->8) -- each depth change is a new shape `torch.compile` hasn't seen, forcing a recompile under `mode="max-autotune"`, which section 8.1's own diagnostic already showed can be genuinely slow (many unique matmul/bmm shapes, each independently autotuned). Several such recompiles across one 25M-token run plausibly consume the entire steady-state per-step win. **Correction to section 8.1/20's recommendation**: `--compile-training` is not a free ~2x in its current form on a curriculum-trained run -- it would need either `mode="default"` (faster to (re)compile, slower steady-state, untested here) or `torch._dynamo.config.dynamic_shapes`-style handling of the depth argument (untested) before it's a real win outside the fixed-depth case the profiler measured.
