...
## 20.3 Real training-budget validation, 2026-08-26 — the compound win survives, but is far smaller than the 5M-token toy scale suggested

Directly requested as the top priority after the full efficiency investigation, in these words: *"does the compound architecture keep its quality advantage at a real training budget? That is now much more important than squeezing another 10-20% out of serving."* Every quality number for the compound architecture up to this point (val_loss 1.7907 vs baseline 1.8585, a 0.068 margin) came from this session's own 5M-token quality-check convention — explicitly a fast, relative-comparison scale, not a claim about real training. This project's own established "real training budget" precedent (Phase F, `docs/restart/hz0h_phase_f_same_gpu_comparison_results.md`) is 25M tokens — 5x tonight's scale. Real runs dispatched at that budget, production shape (n_embd=2496, mult=16, n_layer=8, n_head=8), seed=7, same data/methodology as every other run tonight, one real training pod, ~2.7 hours combined wall-clock:

| | 5M tokens (tonight's scale) | 25M tokens (real budget) |
|---|---:|---:|
| exact BDH baseline | 1.8585 | **1.4383** (`results/local/hz0h_bdh_checkpoint_25m_eval.json`) |
| compound (VB d_state=624 + subspace r=64) | 1.7907 | **1.4326** (`results/local/hz0h_vb_subspace_decoder_25m.json`) |
| margin (compound − baseline) | −0.0678 (−3.65% relative) | **−0.0057 (−0.40% relative)** |

Both numbers drop enormously from 5M to 25M tokens (baseline: 1.8585 -> 1.4383, a real 0.42 improvement) — real, expected confirmation that 5M tokens was a severely undertrained regime for a ~300M-param model, exactly the caveat this session's own quality-check methodology always carried but rarely had a real point of comparison for until now.

**The compound architecture still wins at 25M tokens — the effect is real, not a 5M-token artifact.** But the margin shrank to **8.4% of its 5M-token absolute size** (0.0057 vs 0.0678) and **10.8% of its relative size** (0.40% vs 3.65%). This is the single most important quality correction of the night: every "compound beats baseline by ~0.06-0.07" framing in this document (including the README) was accurate *at 5M-token scale* but substantially overstates the real-training-budget effect size.

**Real, disciplined caveat, not glossed over: this 25M-token result is a single seed.** This session's own cross-seed checks at 5M-token scale found real seed-to-seed noise of 0.017-0.020 for both the baseline (1.8585 vs 1.8789, seed 7 vs 13) and the compound model (1.7907 vs 1.8077) — noise substantially LARGER than the entire 25M-token margin (0.0057) reported here. Training noise often shrinks with more tokens, so the true 25M-token seed variance may well be smaller than the 5M-token figure, but that has not been measured. **Without a second seed at 25M tokens, this result should be read as "directionally consistent with a real, small, positive effect" — not as statistically confirmed at the same confidence level as the earlier 5M-token, 2-seed-checked claims.** A same-budget seed=13 rerun is the obvious, well-scoped next step if this question needs a firmer answer; not yet done tonight given the real time cost (~1.35 hours per additional 25M-token arm).

Real, secondary methodological note: the compound run's SVD-warmstart source was still the original 5M-token-trained checkpoint (`hz0h_bdh_checkpoint_for_ablation.pt`), not the newly-trained 25M-token baseline checkpoint produced alongside it — the warmstart is only an initialization, and the compound model still trains for the full 25M-token budget from there, so this doesn't invalidate the comparison, but a stronger/deeper-trained warmstart source is a real, untested, cheap-to-try follow-up that could plausibly widen the margin back out somewhat.

## 20.4 Phase-binding diagnostic, 2026-08-26 — decisive negative: BDH's native x_latent/y_latent phase is not a dynamic binding code

Requested as step 1 of a 5-step test plan for a proposed "phase-bound synaptic memory" architecture (rotate only the VALUE before writing to state, `S_t = S_{t-1} + k_t^T R(phi_t)v_t`, explicitly NOT touching the addressing/Q,K path — consistent with [[20.1]]'s addressing-vs-output-compression principle). Step 1 was explicitly a zero-architecture-change diagnostic on the existing trained checkpoint (`hz0h_bdh_checkpoint_for_ablation.pt`), asking whether BDH's own pre-ReLU `x_latent`/`y_latent` pair at the last recurrent round already encodes phase-like structure worth exploiting, via `phase = atan2(y_latent, x_latent)`, `amplitude = sqrt(x_latent^2 + y_latent^2)`. Real run, production scale (16,320 real tokens sampled, nh=8, N=4992), script `scripts/hz0h_bdh_phase_binding_diagnostic.py`, verified bit-exact against a manual reference reimplementation of `BDH.forward` before trusting any result. Result: `results/local/hz0h_bdh_phase_binding_diagnostic.json`.

| question | real result | null / comparison | verdict |
|---|---:|---:|---|
| Q1: per-neuron phase circular variance (445 sampled active neurons) | 0.0461 | 0=fixed phase, 1=uniform/random phase | near-fixed, not content-varying |
| Q2: PLV, top co-active pairs (n=2000) vs random pairs (n=580) | 0.9346 | 0.9423 (random) | co-active pairs are NOT more phase-locked than random — slightly less |
| Q3: relative-phase drift across 4 sequential context buckets | 0.00039 rad max-min | large drift would mean context-dependent | essentially frozen, no context modulation |
