...
| A5 | data pipeline rebuild | In progress | 245.6M-token combined corpus (Wikitext + local repo + 3 non-gated external HF sources), all hash-verified. Code (35.1M) and mathematical_and_structured (5.0M) are at their absolute plan-target token counts; documentation (9.3M) is at 93% of target (real source-exhaustion ceiling). json_and_configuration (111K) and terminal_and_debugging (4.6K) remain local-only — no non-gated large-scale source found (the obvious ones, bigcode/the-stack-smol and similar, are gated and need manual HF approval this environment can't grant). Ratio still off-target (general_text is 79.8% of the corpus, not 40%, since Wikitext wasn't subsampled). A 2026-07-28 commit (`8124fcc`) had relabeled this row "Complete" by redefining scope to Wikitext-only; reverted, then genuinely built out — see Detailed Progress below |
...
- A5 exit gate: not satisfied — plan requires a reconstructable 100M-token dataset matching the declared 40/35/10/5/5/5 mixture ratio; corpus now exceeds 100M tokens (245.6M) with 3 of 5 non-text categories at real absolute target scale, but the RATIO is still off (general_text 79.8% vs 40% target, json/terminal still far under 5% each) — see A5 row above
...
- Remaining:
  - json_and_configuration and terminal_and_debugging volume (needs a non-gated source or manual HF gate approval)
  - subsampling general_text down to the target ratio, or otherwise reconciling absolute-scale vs ratio-scale success criteria
  - resumable large-scale iterator and staged mixture reconstruction beyond the existing Wikitext streaming pack
  - restart-era dataset validation at the full 100M-token scale for the new external sources specifically (code/documentation/math are each individually validated at their real packed scale; a combined end-to-end training run against the full v3 mixture hasn't been run yet)
