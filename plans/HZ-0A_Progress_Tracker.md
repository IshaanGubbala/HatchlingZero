...
| A5 | data pipeline rebuild | Substantially satisfied, one category short | 97.1M-token ratio-corrected corpus (Wikitext deliberately subsampled + local repo + 5 non-gated external HF sources), all hash-verified. Actual mixture: general_text 40.5% (target 40), code 36.2% (target 35), documentation 9.6% (target 10), terminal_and_debugging 5.15% (target 5), mathematical_and_structured 5.15% (target 5), json_and_configuration 3.4% (target 5, real source-exhaustion shortfall). 5 of 6 categories within ~1 point of target; only json_and_configuration meaningfully short. A 2026-07-28 commit (`8124fcc`) had relabeled this row "Complete" by redefining scope to Wikitext-only; reverted, then genuinely built out over two passes — see Detailed Progress below |
...
- A5 exit gate: substantially satisfied, not perfect — plan requires a reconstructable 100M-token dataset matching the declared 40/35/10/5/5/5 mixture ratio. Corpus is 97.1M tokens (97% of target) with actual ratios 40.5/36.2/9.6/3.4/5.15/5.15 against target 40/35/10/5/5/5 — 5 of 6 categories within ~1 percentage point, json_and_configuration measurably short (3.4% vs 5%, a real source-exhaustion ceiling in the one non-gated JSON-heavy dataset found, not a shortcut). Calling this "not satisfied" would understate real progress; calling it "satisfied" would overstate it given json_and_configuration's gap — see A5 row above
...
- Remaining:
  - json_and_configuration's ~1.6-point ratio shortfall (needs a second non-gated source, or manual HF gate approval for one of the already-identified gated datasets)
  - resumable large-scale iterator and staged mixture reconstruction beyond the existing Wikitext streaming pack
  - an end-to-end training run against the full v4 mixture hasn't been done yet (code/documentation/math/json/terminal are each individually validated at their real packed scale; Stage 1 training this session used Wikitext-only and the small local-only v2 mixture, not this v4 corpus)
