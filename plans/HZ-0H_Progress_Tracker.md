...
- Overall phase: **H0, H1, and H2 done; T0, T1, and T2 done for all three in-scope architectures (HZ-0A hybrid, BDH-GPU, matched Transformer).** H2 (streaming/chunked equivalence) built and passing on the Torch oracle; the matched Transformer went from "aspirational contract row" to real, tested ternary support this session. Everything past this point (H3-H8, T3-T4) is correctly and honestly still blocked -- see their rows below, not skipped or assumed.
